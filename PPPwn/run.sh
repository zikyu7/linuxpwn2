...
echo -e "\n\033[36m$PITYP\033[92m\nSystem Versi:\033[93m $FIRMWAREVERSION\033[92m\nPort LAN:\033[93m $INTERFACE\033[0m" | sudo tee /dev/tty1
echo -e "\033[92mScript PPPwn:\033[93m C++ $CPPBIN \033[0m" | sudo tee /dev/tty1
if [ $VMUSB = true ] ; then
 sudo rmmod g_mass_storage
  ...
  echo -e "\033[92mFlashdisk:\033[93m Diaktifkan\033[0m" | sudo tee /dev/tty1
fi
if [ $PPPOECONN = true ] ; then
   echo -e "\033[92mKoneksi Internet:\033[93m Diaktifkan\033[0m" | sudo tee /dev/tty1
else   
   echo -e "\033[92mKoneksi Internet:\033[93m Dinonaktifkan\033[0m" | sudo tee /dev/tty1
fi
...
if [[ ! $(ethtool $INTERFACE) == *"Link detected: yes"* ]]; then
   echo -e "\033[31mMenunggu Ps4 Terhubung\033[0m" | sudo tee /dev/tty1
   while [[ ! $(ethtool $INTERFACE) == *"Link detected: yes"* ]]
   do
      coproc read -t 2 && wait "$!" || true
   done
   echo -e "\033[32mPS4 Terhubung\033[0m\n" | sudo tee /dev/tty1
fi
...
if [[ $GHT == *"open"* ]] ; then
echo -e "\n\033[95mHen Sudah aktif , membatalkan Aktivasi\033[0m\n" | sudo tee /dev/tty1
if [[ $LEDACT == "status" ]] ;then
	echo none | sudo tee /sys/class/leds/PWR/trigger >/dev/null
...
echo -e "\n\033[95mHen Belum aktif , memulai aktivasi\033[0m\n" | sudo tee /dev/tty1
sudo killall pppoe-server
if [ $USBETHERNET = true ] ; then
	echo '1-1' | sudo tee /sys/bus/usb/drivers/usb/unbind >/dev/null
	coproc read -t 1 && wait "$!" || true
...
echo -e "\033[31m\nSystem Versi tidak cocok: $FIRMWAREVERSION \033[0m\n" | sudo tee /dev/tty1
if [[ $LEDACT == "status" ]] ;then
	echo none | sudo tee /sys/class/leds/ACT/trigger >/dev/null
...
echo -e "\n\033[95m[PS4 TERHUBUNG] Proses Aktivasi --- > [ + ] Silahkan tunggu\033[0m\n" | sudo tee /dev/tty1
while [ true ]
do
if [[ $LEDACT == "status" ]] ;then
...
do 
 ...
 if [[ $stdo  == "[+] Done!" ]] ; then
	...
	exit 0
 	...
 	echo -e "\033[31m\nGagal , Mengulangi Proses...\033[0m\n" | sudo tee /dev/tty1
